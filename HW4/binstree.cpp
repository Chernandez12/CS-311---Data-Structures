...
void BST::InsertVertex(elem_t E)
{
    ...
    else  // the tree is not empty
    {
        ...
        if (N->Elem < Parent->Elem){
            ...
            N = Parent->Up;       // ** N should point UP to the Parent
...
void BST::DeleteVertex(elem_t E)
{
    ...
    else if ((E == Root->Elem) && ((Root->Left != NULL && Root->Right == NULL) || (Root->Right != NULL && Root->Left == NULL)))
    {
...
//--------------------------------------------------------------------------------
    else if (V->Left != NULL && V->Right == NULL)// ** if V has just the left child so bypass V (case 2)
    {
        ...
        if (V->Left->Elem < P->Elem) {  // ** Make Parent left or right point to the left child and delete V
            P->Left = V->Left;
            P = V->Up;
