...
public:
    ...
    // will enter the given visit number for a given vertex to indicate the order in which vertices were visited.
    void visit(int, char);
    
    // returns true if a given vertex was already visited (0 means not visited)
    bool isMarked(char);
